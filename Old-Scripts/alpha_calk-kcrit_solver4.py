...
def compute_all_profiles(alpha_vals):
    ...
    if to_compute:
        ...
        for res in results:
            if res is not None:
                a, E_core, E_em_base, I2, I4, I0, k = res
    ...
    for a in alpha_vals:
        if a in cache:
            ...
            print(f"Пропущено α={a:.6f} (ошибка вычисления)")
    return data
...
if __name__ == "__main__":
    alpha_vals = np.linspace(0.005, 0.010, 30)

    print("Шаг 1: Вычисление базовых профилей и интегралов...")
    data = compute_all_profiles(alpha_vals)

    # Подбираем K перебором с малым шагом
    K_candidates = np.linspace(500, 2000, 151)
    best_K = None
    best_diff = np.inf
    best_alpha_min = None

    print("Шаг 2: Подбор K...")
    ...
    print("\n" + "="*60)
    print(f"Оптимальное K = {best_K:.2f}")
    print(f"Минимум при этом K: α = {best_alpha_min:.8f}")
    print(f"Экспериментальное α = {alpha_exp:.8f}")
    print(f"Отклонение: {best_diff/alpha_exp*100:.4f}%")
    print("="*60)
    ...
    plt.plot(alphas, energies, 'o-', color='cyan', lw=2, label=f'E_total (K={best_K:.1f})')
    plt.axvline(best_alpha_min, color='yellow', linestyle='--', label=f'Минимум α={best_alpha_min:.6f}')
    plt.axvline(alpha_exp, color='red', linestyle=':', label=f'1/137 = {alpha_exp:.6f}')
    plt.xlabel('α')
    plt.ylabel('E_total')
    plt.legend()
    plt.grid(alpha=0.3)
    plt.title('Энергия электрона-хопфиона с оптимальной нормировкой')
    plt.tight_layout()
    plt.savefig('energy_final_opt.png', dpi=150)
    plt.show()
