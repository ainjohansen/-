...
def relax_soliton(alpha, device, worker_name):
    # K-пространство для уравнения Пуассона (Электромагнетизм)
    ...
    for step in range(STEPS):
        ...
        # 2. Интеграл Скирма (Базовый)
        cross_yz = torch.cross(grad_y, grad_z, dim=0)
        cross_zx = torch.cross(grad_z, grad_x, dim=0)
        cross_xy = torch.cross(grad_x, grad_y, dim=0)
        E_sk_base = 0.25 * torch.sum(cross_xy**2 + cross_yz**2 + cross_zx**2) * dV
        
        # 3. Интеграл Потенциала (Масса)
        # ИСПРАВЛЕНИЕ: Вакуум в n3 = -1, поэтому потенциал 1 + n3
        E_pot = 3 * alpha * torch.sum(1 + n[2]) * dV
        
        # --- ТЕОРЕМА ДЕРРИКА (ВИРИАЛЬНАЯ СТАБИЛИЗАЦИЯ) ---
        # Как в вашем 1D коде: k = (I2 + 3*alpha*I0) / I4
        k_crit = (E_dir + 3 * E_pot) / E_sk_base.clamp(min=1e-8)
        E_sk = k_crit.detach() * E_sk_base 
        
        # 4. Топологический Электромагнетизм
        ...
        # Инвариантная плотность заряда (строго > 0)
        rho = np.sqrt(4 * np.pi * alpha) * (B_mag / (8 * np.pi))
        
        # Решаем Пуассона через FFT: \Delta \Phi = -rho
        rho_fft = torch.fft.fftn(rho)
        Phi_fft = rho_fft / K2
        Phi_fft[0,0,0] = 0.0
        Phi = torch.fft.ifftn(Phi_fft).real
        
        # Кулоновское распирание Тора
        ...
        # --- ЗАМОРОЗКА ГРАНИЦ (Torus Lock) ---
        # Не даем полю "вытечь" за пределы ящика
        with torch.no_grad():
            w.grad[:, 0, :, :] = 0; w.grad[:, -1, :, :] = 0
            w.grad[:, :, 0, :] = 0; w.grad[:, :, -1, :] = 0
            w.grad[:, :, :, 0] = 0; w.grad[:, :, :, -1] = 0
            
        torch.nn.utils.clip_grad_norm_([w], max_norm=2.0)
        optimizer.step()
        scheduler.step()
        
        # Отслеживаем "Массу" тора (интеграл потенциала)
        # Если он > 0, узел жив.
        torus_mass = torch.sum(1 + n[2]) * dV
        
        with torch.no_grad():
            if E_tot.item() < best_E:
                best_E = E_tot.item()
                stable_mass = torus_mass.item()
                best_comps = {'E_dir': E_dir.item(), 'E_sk': E_sk.item(), 'E_pot': E_pot.item(), 'E_em': E_em.item()}
                
        if step % 100 == 0 and worker_name == "GPU-0":
            print(f"   [{worker_name}] Шаг {step:3d} | E: {E_tot.item():.2f} | Объем тора: {torus_mass.item():.2f} | E_em: {E_em.item():.3f}")
            
    print(f"[{worker_name}] α={alpha:.5f} | E={best_E:.2f} | Топология={stable_mass:.1f} | E_em={best_comps['E_em']:.3f}")
