"""
======================================================================
  3D ТОПОЛОГИЧЕСКАЯ МОДЕЛЬ ЛЕПТОНОВ (ФИНАЛЬНАЯ ВЕРСИЯ)
  Оптимальные параметры: L=8.0, scale=1.2, сетка 80^3.
  Строит сечения и изоповерхности.
======================================================================
"""
...
# =====================================================================
GRID_SIZE = 100
L = 8.0                     # проверенный размер области
INIT_SCALE = 1.2            # исходный масштаб тора
...
MAKE_SLICES = True
MAKE_ISOSURFACE = True      # требует plotly
SAVE_FIELDS = True
# =====================================================================

def init_hopfion(device, n=1, scale=INIT_SCALE):
...
def optimize_on_device(device_id, n):
    ...
    with torch.no_grad():
        ...
        n3_center = n_final[2, center_idx, center_idx, center_idx].item()
        print(f"[n={n}] n3 в центре: {n3_center:.3f} (должно быть ~ -1)")
...
def plot_slices(field, n, L, save_prefix='slice'):
    ...
    im0 = axes[0].imshow(n3[mid, :, :], extent=[-L, L, -L, L], origin='lower', cmap='RdBu', vmin=-1, vmax=1)
    axes[0].set_title('XY (z=0)')
    axes[0].set_xlabel('X'); axes[0].set_ylabel('Y')
    plt.colorbar(im0, ax=axes[0])
    
    im1 = axes[1].imshow(n3[:, mid, :], extent=[-L, L, -L, L], origin='lower', cmap='RdBu', vmin=-1, vmax=1)
    axes[1].set_title('XZ (y=0)')
    axes[1].set_xlabel('X'); axes[1].set_ylabel('Z')
    plt.colorbar(im1, ax=axes[1])
    
    im2 = axes[2].imshow(n3[:, :, mid], extent=[-L, L, -L, L], origin='lower', cmap='RdBu', vmin=-1, vmax=1)
...
def plot_isosurface(field, n, L, iso_val=0.0):
    try:
        ...
        return
    n3 = field[2]
    X, Y, Z = np.mgrid[-L:L:complex(0,n3.shape[1]),
                       -L:L:complex(0,n3.shape[2]),
                       -L:L:complex(0,n3.shape[0])]
    ...
    fig.update_layout(title=f'Хопфион n={n} (изоповерхность n3=0)',
...
if __name__ == "__main__":
    ...
    if num_gpus == 0:
        raise RuntimeError("Не найдено GPU.")
    print(f"Доступно GPU: {num_gpus}")
    print(f"Сетка: {GRID_SIZE}^3, L={L}, scale={INIT_SCALE}")
    ...
    if MAKE_SLICES:
        for i, n in enumerate(df['n']):
            plot_slices(fields[i], n, L)
    if MAKE_ISOSURFACE:
        for i, n in enumerate(df['n']):
            plot_isosurface(fields[i], n, L)

    plt.style.use('dark_background')
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12,5))
    ax1.plot(df['n'], df['E'], 'o-', color='cyan', markersize=8)
    ax1.set_xlabel('n'); ax1.set_ylabel('E'); ax1.grid(alpha=0.3)
    ax2.plot(df['n'], df['I'], 'o-', color='magenta', markersize=8)
    ax2.set_xlabel('n'); ax2.set_ylabel('I'); ax2.grid(alpha=0.3)
