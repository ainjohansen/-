...
print("="*75)
print(" ТОПОЛОГИЧЕСКАЯ МОДЕЛЬ: ПРОТОН, НЕЙТРОН И ЗАДЕЛ ДЛЯ БЕТА-РАСПАДА")
print("="*75)
...
# Используем новое имя кэша, так как математика узла изменилась
cache_filename = f"cache_hopfion_v2_grid_{grid_size}.npz"
...
# =====================================================================
# Функция генерации каркаса с параметром вкручивания хопфиона (beta_phase)
def generate_nucleon_curve(beta_phase, num_points=15000):
    """
    beta_phase = 0.0 -> Чистый протон
    beta_phase = 1.0 -> Чистый нейтрон (вкручен 1 хопфион)
    0 < beta_phase < 1 -> Динамика слабого распада (вывинчивание)
    """
    ...
    if beta_phase > 0:
        # Глобальное, ИДЕАЛЬНО ГЛАДКОЕ И СИММЕТРИЧНОЕ скручивание всей трубки.
        # Никаких локальных дефектов. Частота кратна для идеального замыкания контура.
        twist_freq = 15  
        twist_amp = 0.08636 * beta_phase 
...
if os.path.exists(cache_filename):
    print(f"\n[!] Найден кэш '{cache_filename}'. Расчетная сетка совпадает.")
    ...
    mid_z_index = grid_size // 2 # Индекс центрального среза
    ...
    for i, z in enumerate(z_g):
        ...
        # --- ИНТЕГРАЛ НЕЙТРОНА (ГЛОБАЛЬНО СКРУЧЕННЫЙ) ---
        ...
        # Сохранение экваториального среза для тепловой карты
        if i == mid_z_index:
...
print("ВЫВОД: Нейтрон не имеет сингулярных 'разрывов'. Вся масса паркуется")
print("в равномерном глобальном скручивании всей топологической трубки.")
print("=====================================================================")

# =====================================================================
# 5. ВИЗУАЛИЗАЦИЯ (ВКЛЮЧАЕТ ЗАДЕЛ ДЛЯ ДИНАМИКИ)
# =====================================================================
print("\nГенерация графиков... Закройте окно для завершения скрипта.")

fig = plt.figure(figsize=(16, 10))
plt.style.use('dark_background')
fig.suptitle(f'Глобальная Симметрия Нуклонов и Скручивание (Сетка: {grid_size}^3)', fontsize=16, fontweight='bold')
...
# Нейтрон рисуем поверх, он будет выглядеть как накрученная спираль на базу
...
ax1.set_title("Наследование геометрии: Вкручивание Хопфиона")
ax1.set_axis_off()
ax1.legend()
...
# Квадратичная или линейная интерполяция энергии в зависимости от фазы кручения 
# (упрощенная модель энергии распада)
